...
async function gobotNotify(text, desp, time = 2100) {
    ...
    let groupFlag = false; // 是否群发消息
    // 群发消息关键字
    let notifyKeywords = ["已可兑换","新的水果","已可领取","选择新商品","失效"];
    for (let notifyKeyword of notifyKeywords) {
        if (desp.indexOf(notifyKeyword) != -1) {
            groupFlag = true;
            break;
        }
    }
    try {
        ...
        for (const user of users) {
            if (user.pt_pin && user.nickName && user.remark) {
                desp = desp.replace(new RegExp(`${user.pt_pin}|${user.nickName}`, 'gm'), `[CQ:at,qq=${user.remark}]`);
            }
        }
    } catch (e) {
        console.error(e);
    }
    let api = groupFlag ? "/send_group_msg" : "/send_private_msg";
    let toID = groupFlag ? "group_id=" + process.env.GOBOT_GROUP : "user_id=" + process.env.GOBOT_QQ;

    return new Promise((resolve) => {
        if (GOBOT_URL) {
            const options = {
                url: `${GOBOT_URL}${api}?access_token=${GOBOT_TOKEN}&${toID}`,
                json: {message: `${text}\n${desp}`},
                headers: {
